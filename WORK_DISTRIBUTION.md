...
| 2026-08-13 | Member 3 step **3.2**: seller status screens — dedicated pending/rejected UX on `/become-seller` (`SellerApplicationStatus`); `/seller` layout redirects pending/rejected applicants to status page via `blockedSellerPortalDestination`; approved gate unchanged (Auth `seller` label); self-check `scripts/verify-seller-status-gate.ts` |
