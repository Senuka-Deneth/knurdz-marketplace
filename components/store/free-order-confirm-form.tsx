...
import {
  confirmFreeOrderAction,
  type ConfirmFreeOrderActionState,
} from "@/lib/services/order-actions";
import type { Order, Payment } from "@/lib/types";
import { toast } from "@/lib/ui/toast";

const initialState: ConfirmFreeOrderActionState = {};

type FreeOrderConfirmFormProps = {
  order: Order;
  payment: Payment;
};

export function FreeOrderConfirmForm({
  order,
  payment,
}: FreeOrderConfirmFormProps) {
  ...
  useEffect(() => {
    ...
    if (state.ok && state.paymentStatus === "paid") {
      ...
      if (key !== lastToast.current) {
        lastToast.current = key;
        toast.success("Your free order is confirmed.");
      }
      router.refresh();
    }
  }, [state, router]);
